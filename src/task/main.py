...
def tuner_setups():
    ...
    if os.getenv("LANCEDB_ENDPOINT"):
        storage_options = {
            ...
            "aws_access_key_id": os.getenv("LANCEDB_ACCESS_KEY"),
            "aws_secret_access_key": os.getenv("LANCEDB_SECRET_KEY"),
    ...
    datasets = vector_store.to_querytasks()
...
async def tuning_process(batch_size, epochs, beam_n, train_rate):
    ...
    try:
        ...
        resources = await store.add_resources({
        ...
        for epoch in range(epochs):
            # run an epoch
            candidate_instructions = await tuner.epoch(datasets=datasets, beam_n=beam_n, batch_size=batch_size, epoch=epoch, train_rate=train_rate)
            # pick instruction
            instruction = tuner.evolve()
            epoch_reward = instruction.origin_reward + instruction.delta_reward
            # update templates to resources
            updated_resources = await store.add_resources({
                "prompt_template": evaluator.prompt_template,
                "instruction": PromptTemplate(
                    template=json.dumps({
        ...
        if 'store' in locals() and store:
            await store.close()
            console.print(f"{marker} Store closed in main.")
    ...
    try:
        ...
        pass
    file_path = f"./storage/history-{tuner_id}.parquet"
    pq.write_table(history_df, file_path)
    console.print(f"{marker} Save history to storage. Path: {file_path}")
    pass


@click.command()
@click.option('--batch-size', default=2, help='Batch size for training', type=int, show_default=True)
@click.option('--epochs', default=3, help='Number of training epochs', type=int, show_default=True)
@click.option('--beam-n', default=2, help='Beam search width', type=int, show_default=True)
@click.option('--train-rate', default=0.05, help='Training rate', type=float, show_default=True)
def main(batch_size, epochs, beam_n, train_rate):
    asyncio.run(tuning_process(batch_size, epochs, beam_n, train_rate))

if __name__ == "__main__":
    from dotenv import load_dotenv
    from agentlightning import PromptTemplate, LightningStoreClient
