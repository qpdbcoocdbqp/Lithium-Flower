...
class RAGOptimizer(APOptimizerBase):
    ...
    async def critique(self, inputs: dict) -> Critique:
        messages = [{"role": "system", "content": self.critique_template.prompt.template.format(**inputs)}]
    ...
    async def rewrite(self, critiques: dict) -> Rewrite:
        messages = [{"role": "system", "content": self.rewrite_template.prompt.template.format(**critiques)}]
